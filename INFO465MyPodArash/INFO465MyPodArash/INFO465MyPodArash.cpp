// INFO465MyPodArash.cpp : Defines the entry point for the console application.
//
...
void List::Display()
{
	...
	if (tmp == NULL)
	{
		cout << "Oh No would you look at that the List is Dry (Empty)" << endl;
		return;
	}
	if (tmp->Type() == NULL)
	{
...
void List::playSong()
{
	...
	if (tmp == NULL)
	{
		cout << "WOW Nothing is here for you, Sorry" << endl;
		return;
	}
	if (tmp->Type() == NULL)
	{
	...
	else
	{
		do
		{
			if (songNumber == i)
			{
				cout << " Currently" << songNumber << " playing Song " << endl;
				cout << i;
				cout << ":";
				cout << tmp->File() << endl;
				cout << endl << endl;
...
int main()
{
	...
	do
	{
		...
		if (ans == '2')
		{
			...
			{
				cout << "You had one job... try agian *** with a different number ;) ***";
			}
		}
		if (ans == '3') {
